        ...
        <>
            <div className="grid">
                <Dialog visible={visible} maximizable style={{ width: '50vw' }} onHide={() => setVisible(false)}>
                    <AddAndEditSubTopic subTopics={subTopics} setSubTopics={setSubTopics} isNew={false} subTopic={subTopic} />
                </Dialog>
                <div className="col-12">
                    {subTopics.length > 0 && <DataTableRenderer<SubTopic> data={subTopics} tableColumns={tableColumns} />}
                </div>
            </div>
        </>
    )
