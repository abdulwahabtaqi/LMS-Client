        ...
        <>
            <div className="grid">
                <Dialog visible={visible} maximizable style={{ width: '50vw' }} onHide={() => setVisible(false)}>
                    <AddAndEditTopic topics={topics} setTopics={setTopics} isNew={false} topic={topic} />
                </Dialog>
                <div className="col-12">
                    {topics.length > 0 && <DataTableRenderer<Topic> data={topics} tableColumns={tableColumns} />}
                </div>
            </div>
        </>
    )
